...
import {getCartNum} from '../ts/HttpRequests/LayoutRequest'
export default {
    setup () {
     ...
     const onGetCartNum = async()=>{
      ...
      let cartNum = await getCartNum();
      store.dispatch("shoppingCart/setCartNum",cartNum); //vue2用法是 this.$store.dispatch()
    }
    onMounted(async()=>{
      await onGetCartNum();
    })
    ...
    .foot-item {
      ...
      b {
        ...
        i {
          ...
          font-size: 12px;
          color: #fff;
          background-color: rgb(71,158,211);
          padding: 1px 3px;
          text-align: center;
          font-style: normal;
          display: inline;
